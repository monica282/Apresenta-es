    ...
    <div className="min-h-screen flex bg-gradient-to-br from-[#0a0f3d] via-[#1f1c52] to-[#40318d] text-white">
      <aside className="w-64 bg-[#0a0f3d] p-6 flex flex-col items-start gap-4 border-r border-indigo-700">
        ...
        {slides.map((slide, i) => (
          <button
            ...
            onClick={() => setIndex(i)}
            aria-current={i === index ? "page" : undefined} // Adiciona atributo ARIA para acessibilidade
          >
            {slide.title}
          </button>
        ))}
      ...
      <main className="flex-1 p-10 flex flex-col items-center justify-center">
        ...
        >
          ...
          <p className="text-lg leading-relaxed whitespace-pre-wrap">{slides[index].content}</p> {/* Alterado para whitespace-pre-wrap */}
        </motion.div>
      </main>
    </div>
  );
